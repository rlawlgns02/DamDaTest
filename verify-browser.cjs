...
(async()=>{
 ...
 async function checkRatios(page,selector){const sizes=await page.locator(selector).evaluateAll(els=>els.map(el=>({w:parseFloat(getComputedStyle(el).width),h:parseFloat(getComputedStyle(el).height)})));assert(sizes.length);for(const size of sizes)assert(Math.abs(size.w/size.h-1.8)<.02,JSON.stringify(size));}
 ...
 try{
  ...
  await page.screenshot({path:'test-results/login.png',fullPage:true});assert.equal(await page.locator('.art-grid').count(),0);assert.match(await page.locator('.art-card-name').textContent(),/^Hello/);for(const width of [320,390,430,600,768,1440]){await page.setViewportSize({width,height:844});assert(await page.locator('.art-card').isVisible());await checkRatios(page,'.art-card');assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));}await page.screenshot({path:'test-results/login-tablet.png',fullPage:true});await page.setViewportSize({width:390,height:844});await page.screenshot({path:'test-results/login-mobile.png',fullPage:true});await page.setViewportSize({width:1440,height:1000});
  ...
  for(const action of ['download-front','download-back','download-vcf']){const wait=page.waitForEvent('download');await page.locator('[data-action='+action+']').click();const dl=await wait;assert.equal(await dl.failure(),null);if(action!=='download-vcf'){const png=await fs.readFile(await dl.path());assert.equal(png.readUInt32BE(16),1440);assert.equal(png.readUInt32BE(20),800);}}
  ...
  for(const [width,height] of [[320,740],[390,844],[844,390],[768,1024],[1024,768],[1366,900],[1920,1080]]){
   await page.setViewportSize({width,height});
   for(const route of ['home','book','templates']){await page.goto(base+'#'+route);await page.locator('.page-heading').waitFor();await checkRatios(page,'.business-card');assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),route+' overflow at '+width);}
  ...
  await page.locator('[data-model=backTitle]').fill('긴 제목 '.repeat(12));await page.locator('[data-model=backMessage]').fill('자세한 소개 문구입니다. '.repeat(15));await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));await checkRatios(page,'.business-card');assert(await page.locator('#live-card .reverse p').evaluate(el=>el.scrollHeight<=el.clientHeight+1));await page.locator('[data-action=logout]').click();await page.locator('#login-form').waitFor();
